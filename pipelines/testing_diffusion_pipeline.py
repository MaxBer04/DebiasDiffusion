...
class TestingDiffusionPipeline(StableDiffusionPipeline):
    def __init__(self,
                 vae: AutoencoderKL,
    ...
    def pred_image_latent(self, z_t, model_output, timestep):
        alpha_t = self.scheduler.alphas_cumprod[timestep].to(z_t.device)

        z_0_pred = (1 / alpha_t**(0.5)) * (z_t - (1-alpha_t)**(0.5) * model_output)
        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                copied_lat = latents.detach().clone()
                z_0_pred = self.pred_image_latent(copied_lat, noise_pred, int(t))
                z_0_pred_list.append(z_0_pred)
        ...
        if not output_type == "latent":
            ...
            if return_image_predictions:
                img_predictions = []
                for i in range(len(z_0_pred_list)):
                    img_predictions.append(self.vae.decode(z_0_pred_list[i] / self.vae.config.scaling_factor, return_dict=False)[0])
        ...
        if has_nsfw_concept is None:
            ...
            if return_image_predictions:
                do_denormalize_preds = [True] * img_predictions[0].shape[0]
        else:

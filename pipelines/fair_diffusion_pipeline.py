###############################################
###### This is a slightly changed custom
###### implementtation of the semantic
###### diffusion pipeline.
######
###############################################
...
class SemanticStableDiffusionPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        self.edit_mom_beta=None # Momentum beta
        self.edit_guidance_scale=None # Guidance scale for each concept
        
        self.editing_prompts={}
        #self.reverse_editing_direction={} # Direction of guidance i.e. decrease the first and increase the second concept
        self.edit_warmup_steps={} # Warmup period for each concept
        self.edit_thresholds={} # Threshold for each concept. Threshold equals the percentile of the latent space that will be discarded. I.e. threshold=0.99 uses 1% of the latent dimensions
        self.edit_momentum_scales={} # Momentum scale that will be added to the latent guidance
        self.edit_weights={}  # Weights of the individual concepts against each other
    ...
    def set_momentum(self, scale, beta):
        self.edit_momentum_scale = scale
        self.edit_momentum_beta = beta
    
    def set_attribute_params(self, editing_prompts, edit_warmup_steps, edit_guidance_scales, edit_thresholds, edit_weights):
        self.editing_prompts=editing_prompts
        #self.reverse_editing_direction=reverse_editing_direction
        self.edit_warmup_steps=edit_warmup_steps
        self.edit_guidance_scales=edit_guidance_scales
        self.edit_thresholds=edit_thresholds
    ...
    def create_structured_editing_prompts(self, batch_size):
        result = []
        
        # Erstelle eine Liste von Attributwerten für jedes Batch-Element
        batch_attributes = []
        for _ in range(batch_size):
            batch_element = {}
            for attr, prompts in self.editing_prompts.items():
                weights = self.edit_weights[attr]
                chosen_prompt = random.choices(prompts, weights=weights, k=1)[0]
                batch_element[attr] = chosen_prompt
            batch_attributes.append(batch_element)
        
        # Erstelle die strukturierte Liste
        for attr, prompts in self.editing_prompts.items():
            for i in range(batch_size):
                result.append(batch_attributes[i][attr])
            for i in range(batch_size):
                batch_attributes[i][attr] = [batch_attributes[i][attr]]
            reamining_classes = 3 if attr == "race" else 1
            for k in range(reamining_classes):
                for i in range(batch_size):
                    remaining_prompts = [pr for pr in prompts if pr not in batch_attributes[i][attr]]
                    if remaining_prompts:
                        batch_attributes[i][attr].append(remaining_prompts[0])
                    result.append(remaining_prompts[0])
    ...
    def __call__(
        ...
        edit_threshold = []
        edit_weights = []
        for attr in self.editing_prompts.keys():
            for concept in self.editing_prompts[attr]:
                editing_prompt.append(concept)
            for scale in self.edit_guidance_scales[attr]:
                edit_guidance_scale.append(scale)
            for steps in self.edit_warmup_steps[attr]:
                edit_warumup_steps.append(steps)
            for thr in self.edit_thresholds[attr]:
                edit_threshold.append(thr)
            for weight in self.edit_weights[attr]:
                edit_weights.append(weight)
        ...
        if enable_edit_guidance:
            ...
            if editing_prompt_embeddings is None:
                test = self.create_structured_editing_prompts(batch_size)#[x for item in editing_prompt for x in repeat(item, batch_size)]
                #print(self.create_weighted_editing_prompts(batch_size))
